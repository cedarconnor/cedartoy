...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    vec2 p = ((fragCoord.xy * 2.0 - iResolution.xy) / min(iResolution.x, iResolution.y)) * zoomMul;

    // === kick_pulse_camera (cookbook_version 1) ===
    float kickEnergy = texture(iChannel0, vec2(0.03, 0.25)).r;

    vec3 cPos = vec3(0.0, 0.0, -3.0 * iTime - kickEnergy * 0.08);
    ...
    // Rotate the (R, G, B) channel weights slightly by the palette index so
    // each section reads as a different colour wash without losing identity.
    vec3 baseCol = vec3(acc * 0.01, acc * 0.011 + acc2*0.002, acc * 0.012 + acc2*0.005);
    float hue = palette * 0.6;
    mat3 hueRot = mat3(
        cos(hue), -sin(hue), 0.0,
        sin(hue),  cos(hue), 0.0,
        0.0,       0.0,      1.0
    );
    vec3 col = mix(baseCol, hueRot * baseCol, 0.5);
