...
// -- Brainfiller Ported for Cedartoy --
// Original by 0b5vr: https://github.com/0b5vr/brainfiller
// Ported to standard Shadertoy/Cedartoy format
...
// const float PI = acos(-1.0); // Defined in header
...
// Cedartoy/Shadertoy standard entry point
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
  ...
  vec3 rnd_seed = hash3f_custom(vec3(p, time));
  ...
  for (int i = 0; i ++ < SAMPLES_PER_FRAME;) {
    ...
    vec3 ro = orthBas(colRem) * vec3(1.5 * (p + (rnd_seed = hash3f_custom(rnd_seed)).xy / iResolution.y) + vec2(0, 0.1 * time), 6.0) - vec3(0.0, 0.0, 2.0);
    ...
    for (int i = 0; i ++ < TRAVERSAL_STEPS;) {
...

      {
        ...
        if (ro.z < 0.0) {
          ...
          for (int i = 0; i ++ < 5; ) {
            ...
            cellDice = hash3f_custom(cell + clamp(gen, 31.0, 288.0));
          }
        }
...

        {
          ...
          cellDice = hash3f_custom(cellDice);
          ...
          if (cell.z != 0.0) {
            ...
            if (isect2.w < isect.w) {
              ...
              float mask = step(max(abs(ncoord.x), abs(ncoord.y)), 0.9) * (
                ...
                cellDice.y < 0.8 ? step(hash3f_custom(floor(ncoord.xyy / 0.45) + cellDice).x, 0.5) * step(length(fract(ncoord / 0.45) - 0.5), 0.4) :
            ...
            if (isect2.w < isect.w) {
              ...
              vec3 grafix =
                ...
                cellDice.y < 0.95 ? vec3(step(hash3f_custom(floor(coord * 256.0) + floor(beat * 4.0) + cellDice).x, 0.4)) :
                vec3(0.02, 0.02, 1.0);
...

      {
        ...
        float spec = max(
          step((rnd_seed = hash3f_custom(rnd_seed)).x, Fn), // non metallic, fresnel
          i_metallic // metallic
        );

        // sample ggx or lambert
        rnd_seed.y = sqrt( ( 1.0 - rnd_seed.y ) / ( 1.0 - spec * ( 1.0 - sqSqRoughness ) * rnd_seed.y ) );
        vec3 woOrH = orthBas( isect.xyz ) * vec3(
          sqrt( 1.0 - rnd_seed.y * rnd_seed.y ) * sin( TAU * rnd_seed.z + vec2( 0.0, TAU / 4.0 ) ),
          rnd_seed.y
        );
  ...
  // --- Feedback Mix ---
  // Mix current frame with previous frame (backBuffer -> iChannel0)
  outColor = mix(
