...
const float PI_VAL = acos(-1.0); // Renamed to avoid header conflict
const float TAU = PI_VAL + PI_VAL;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
  ...
  for (int i = 0; i ++ < SAMPLES_PER_FRAME;) {
    ...
    // Camera calc
    mat3 bas = orthBas(colRem); 
    // orthBas(z) returns [x, y, z] columns.
    // Original:
    vec3 ro = bas * vec3(1.5 * (p + (seed = hash3f(seed)).xy / iResolution.y) + vec2(0, 0.1 * time), 6.0) - vec3(0.0, 0.0, 2.0);
    vec3 rd = bas * vec3(0.0, 0.0, -1.0);

    // CedarToy VR Logic
    if (iCameraMode > 0) {
        // Construct Basis
        // Forward is -z in original logic?
        // Original rd is bas * (0,0,-1) = -bas.z
        // So bas.z is BACK?
        // Let's redefine basis for VR helper.
        // Helper expects Right, Up, Forward.
        // bas.x is Right? bas.y is Up? bas.z is Back?
        // Let's check orthBas: z=input. x=norm(cross(up, z)). y=cross(z,x).
        // If z is back, x is right (up x back = right? Y x Z = X. Correct).
        // y is up (back x right = up? Z x X = Y. Correct).
        // So bas is [Right, Up, Back].
        // We want [Right, Up, Forward].
        // So Forward = -bas.z
        
        vec3 cRight = bas[0];
        vec3 cUp = bas[1];
        vec3 cFwd = -bas[2];
        mat3 vrBasis = mat3(cRight, cUp, cFwd);
        ...
        if (iCameraMode == 1) { // Equirect
             ...
             rd = normalize(vrBasis * dir);
        } else if (iCameraMode == 2) { // LL180
            rd = cameraDirLL180(q, iCameraTiltDeg, vrBasis);
        }
        
        // ro might need adjustment if p logic is weird.
        // Original ro depends on 'p'.
        // In VR, ro should probably be independent of p (pixel) for the "Center", but stereo might need offset.
        // But here loop accumulates samples.
        // For simplicity, we keep ro logic but use center uv (0.5) or just `p` (which is varying direction in 2D).
        // For accurate VR, ro should be the nodal point.
        // 'ro' here has some jitter based on 'p'. That suggests Depth of Field or area sampling?
        // "1.5 * (p + jitter)"
        // If I change 'rd' but keep 'p' based on screen uv, 'ro' is still jittered.
