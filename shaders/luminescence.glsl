...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    if (iCameraMode == 1) { // Equirectangular
        // Remap UV to avoid 2D aspect ratio correction causing distortion in equirect map
        // FragCoord derived UV in mainImage was: uv -= .5; uv.y *= split...
        // We want raw 0..1 UVs for equirect mapping
        ...
        // uv is 0..1 from fragCoord/iResolution
        vec2 q = fragCoord.xy / iResolution.xy;
        
        // User wants "horizon rotated down 65 degrees".
        // Standard LL180 assumes looking FORWARD (Z+).
        // User image suggests looking DOWN (-Y) or UP (+Y) with a tilt.
        // "Camera facing up" usually means we are looking at the sky.
        // "Rotate horizon down" means we want to look more towards the horizon?
        // Let's re-orient the basis so "Forward" is actually "Up" relative to the world, 
        // or "Down" relative to the world, depending on the desired "Little Planet" effect.
        
        // Re-orient basis: Map Camera Forward to World Down (-Y) to match "planet" look?
        // Or keep standard basis and let iCameraTiltDeg handle the pitch.
        // If the user says "camera is just facing up", maybe our default lookAt was wrong.
        // In mainImage: vec3 lookAt = vec3(0., -1., 0.);
        // CameraSetup points camera at lookAt.
        // cam.forward = normalize(lookAt - cam.p).
        
        // For LL180, we want to override the ray direction purely based on the Lens projection.
        // But the Lens projection needs a Reference Frame (Basis).
        // Standard VR180/Dome usually assumes:
        // Center of lens = Horizon (Forward).
        // Top of lens = Zenith (Up).
        
        // If the query is "rotate horizon down", it implies Pitching the camera UP?
        // Or Pitching the camera DOWN?
        // "Horizon rotated down" -> Horizon moves to bottom of image -> Camera looks UP.
        
        // Let's try to match the reference image style (Little Planet / Dome).
        // Usually, Dome masters are oriented with Zenith at center (looking UP).
        
        // FORCE BASIS:
        // Center of render (0,0) -> World Up (0,1,0).
        // Right -> World Right (1,0,0).
        // Up -> World Back/Forward?
        
        // Let's assume standard Dome Master orientation:
        // Center = Zenith (0,1,0).
