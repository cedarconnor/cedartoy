...
from .modulation import MOD_SOURCES

# Same source vocabulary as the modulation matrix, so a route's source and
# its scorecard row always line up. The pulses are derived from beats here.
DERIVED_SOURCES: Tuple[str, ...] = ("beat_pulse", "downbeat_pulse")
SCORE_SOURCES: Tuple[str, ...] = tuple(
    s for s in MOD_SOURCES if s not in DERIVED_SOURCES)
